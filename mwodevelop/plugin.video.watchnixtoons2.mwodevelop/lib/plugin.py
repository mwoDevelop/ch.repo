...
from lib.playback_sync import apply_cached_playback, notify_profile_identity, notify_profile_sync
...
def actionEpisodesMenu(params):
    ...
    def _episodeItemsGen():
        ...
        for url, title, episode_type in listIter:
            ...
            try:
                apply_cached_playback(item, url)
...
def actionResolve(params):
    ...
    if 'getvid?evid' in html:
        ...
        # requests returns text here. Passing bytes to str.find raises TypeError
        # on Kodi's Python 3 runtime and broke the fallback JWPlayer resolver.
